...
import {
  ...
  IORedisClassType,
  IORedisClientOptions} from '../../types/redis';

export class ConnectorService {
  //1) Initialize `store`, `stream`, and `subscription` Redis clients.
  //2) Bind to the target if not already present
  static async initRedisClients(Redis: Partial<RedisClass>, options: Partial<RedisOptions>, target: HotMeshEngine | HotMeshWorker): Promise<void> {
