  ...
  wrapWorkflowFunction(
    ...
    return async (data: StreamData): Promise<StreamDataResponse> => {
      ...
      try {
        ...
        if (
          err instanceof MeshFlowWaitForError ||
          ...
          return {
            ...
            data: {
              ...
              await: err.await,
              backoffCoefficient:
                err.backoffCoefficient || HMSH_MESHFLOW_EXP_BACKOFF,
              code: err.code,
              index: err.index,
        ...
        return {
          status: StreamStatus.SUCCESS,
          code: err.code || new MeshFlowRetryError(err.message).code,
          metadata: { ...data.metadata },
          data: {
            $error: {
              message: err.message,
