 ...
 * from anywhere on the network connected to the target backend
 * (Postgres, NATS, etc). Invocations can be scheduled to run as
 * idempotent cron jobs (this one runs nightly at midnight and
 * uses Postgres as the backend provider).
 *
 * @example
 * ```typescript
...
class Virtual {
  ...
  static async findFirstMatching(
    ...
    options: VirtualInstanceOptions = {},
    ...
    for (const [id, hotMeshInstance] of targets) {
      ...
      if (appId === namespace) {
        if (id.startsWith(Virtual.hashOptions(config))) {
          if (
  ...
  static getHotMeshClient = async (
    ...
    const optionsHash = Virtual.hashOptions(connection);
    const targetNS = namespace ?? HMNS;
    const connectionNS = `${optionsHash}.${targetNS}`;
    if (Virtual.engines.has(connectionNS)) {
      const hotMeshClient = await Virtual.engines.get(connectionNS);
    ...
    Virtual.engines.set(connectionNS, hotMeshClient);
  ...
  static async verifyWorkflowActive(
    ...
    if (isNaN(appVersion)) {
      ...
      return await Virtual.verifyWorkflowActive(hotMesh, appId, count + 1);
    }
    return true;
  }
  ...
  static async activateWorkflow(
    ...
    if (appVersion === version && !app.active) {
      try {
        ...
        hotMesh.engine.logger.error('virtual-client-activate-err', {
          error,
        });
        throw error;
      }
      ...
      try {
        ...
        hotMesh.engine.logger.error('virtual-client-deploy-activate-err', {
          error,
        });
        throw error;
      }
  ...
  static async getInstance(
    ...
    if (!options.readonly) {
      hotMeshInstance = await Virtual.findFirstMatching(
        Virtual.workers,
        namespace,
        providerConfig,
        options,
    ...
    if (!hotMeshInstance) {
      ...
      if (!hotMeshInstance) {
        hotMeshInstance = (await Virtual.getHotMeshClient(
          namespace,
          providerConfig,
          options,
   ...
   * ```
   */
  static async connect(params: VirtualConnectParams): Promise<HotMesh> {
    const targetNamespace = params.namespace ?? HMNS;
    const optionsHash = Virtual.hashOptions(polyfill.providerConfig(params));
    ...
    Virtual.workers.set(targetTopic, hotMeshWorker);
    await Virtual.activateWorkflow(hotMeshWorker, targetNamespace);
   ...
   * @example
   * ```typescript
   * const response = await Virtual.exec({
   *   topic: 'my.function',
   *   args: [{ my: 'args' }],
   *   connection: {
   *     class: Postgres,
   ...
   * ```
   */
  static async exec<U>(params: VirtualExecParams): Promise<U> {
    const TOPIC = `${params.namespace ?? HMNS}.call`;
    const hotMeshInstance = await Virtual.getInstance(
      params.namespace,
   ...
   * ```
   */
  static async flush(params: VirtualFlushParams): Promise<void> {
    const hotMeshInstance = await Virtual.getInstance(
      params.namespace,
      polyfill.providerConfig(params),
    );
   ...
   * ```
   */
  static async cron(params: VirtualCronParams): Promise<boolean> {
    let hotMeshInstance: HotMesh | void;
    let readonly = true;
    if (params.callback) {
      //always connect cron worker if provided
      hotMeshInstance = await Virtual.connect({
    ...
    try {
      if (!hotMeshInstance) {
        ...
        hotMeshInstance = await Virtual.getInstance(
          params.namespace,
          polyfill.providerConfig(params),
          { readonly, guid: params.guid },
        );
        await Virtual.createStream(
          hotMeshInstance,
          params.topic,
          params.namespace,
        );
   ...
   * ```
   */
  static async interrupt(params: VirtualInterruptParams): Promise<boolean> {
    const hotMeshInstance = await Virtual.getInstance(
      params.namespace,
      polyfill.providerConfig(params),
    );
  ...
  static async shutdown(): Promise<void> {
    for (const [_, hotMeshInstance] of Virtual.workers) {
      (await hotMeshInstance).stop();
    }
    for (const [_, hotMeshInstance] of Virtual.engines) {
      (await hotMeshInstance).stop();
    }
    await HotMesh.stop();
  }
