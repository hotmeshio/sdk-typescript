...
describe('MEMFLOW | hello | `Random Hello-World` | Redis', () => {
  let handle: WorkflowHandleService;
  let workflowId: string;
  let client: ClientService;
  const namespace = 'memflow';
  const options = {
    socket: {
  ...
  describe('WorkflowHandle', () => {
    describe('result', () => {
      it('should return the workflow execution result', async () => {
        const newHandle = await client.workflow.getHandle(
          'hello-world',
        ...
        expect(newResult).toEqual(`${r1} Hello, HotMesh! ${r2}`);
