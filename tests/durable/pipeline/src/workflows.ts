...
export async function documentProcessingPipeline(): Promise<any> {
  const pipeline = await Durable.workflow.entity();
...
export async function pageProcessingHook(imageRef: string, pageNumber: number, documentId: string): Promise<void> {
  ...
  try {
    ...
    await Durable.workflow.signal(`page-${pageNumber}-complete`, {
      pageNumber,
...
export async function validationHook(): Promise<void> {
  ...
  try {
    ...
    await Durable.workflow.signal('validation-complete', { validationResults });
  } catch (error) {
    if (Durable.workflow.didInterrupt(error)) {
      throw error;
    }
...
export async function approvalHook(documentId: string): Promise<void> {
  ...
  try {
    ...
    await Durable.workflow.signal('approval-complete', { approvalResults });
...
export async function notificationHook(documentId: string): Promise<void> {
  ...
  try {
    ...
    await Durable.workflow.signal('processing-complete', { finalResult });
  } catch (error) {
    if (Durable.workflow.didInterrupt(error)) {
      throw error;
    }
