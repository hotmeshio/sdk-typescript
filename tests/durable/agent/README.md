...
### Full Test Suite
```bash
npm run test:durable:agent
```

### Individual Test Components
```bash
# Test basic functionality (no OpenAI required)
NODE_ENV=test jest tests/durable/agent/postgres.test.ts -t "basic functionality"

# Test with OpenAI integration
NODE_ENV=test jest tests/durable/agent/postgres.test.ts -t "complete research agent"
```
